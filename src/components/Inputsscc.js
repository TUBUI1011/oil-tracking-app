...
function InputSsccPage({ onAddTanks }) {
  ...
  const handleManualSave = () => {
    ...
    const newTank = { code: materialCode, sscc, batch };
    onAddTanks([newTank]); // Truyền vào một mảng chứa 1 tank
      ...
      <main className="sscc-main">
        ...
        <div className="form-wrapper">
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="Nhập mã material" value={materialCode} onChange={(e) => setMaterialCode(e.target.value)} />
          </div>
          <div className="form-group">
            <p className="form-label">SSCC</p>
            <input className="form-input" placeholder="Nhập mã SSCC" value={sscc} onChange={(e) => setSscc(e.target.value)} />
          </div>
          <div className="form-group">
            <p className="form-label">Batch</p>
            <input className="form-input" placeholder="Nhập số batch" value={batch} onChange={(e) => setBatch(e.target.value)} />
          </div>
          <button className="save-button" onClick={handleManualSave}>Lưu thông tin</button>
        </div>

        <div className="divider-container">
          <div className="divider-line"></div><span className="divider-text">hoặc</span><div className="divider-line"></div>
        </div>

        {/* --- PHẦN UPLOAD EXCEL --- */}
        <div className="form-wrapper">
          <h2 className="section-title">Tải lên từ file Excel</h2>
          <p className="upload-note">File Excel cần có 3 cột theo thứ tự: Material Code, SSCC, Batch.</p>
        ...
        {excelData.length > 0 && (
          <div className="excel-preview-wrapper">
            <h2 className="section-title">Dữ liệu xem trước ({excelData.length} tank)</h2>
            ...
            <button className="confirm-button" onClick={handleConfirmExcel}>Xác nhận và Thêm vào kho</button>
          </div>
        )}
      </main>
    </div>
